...
const CalendarView = () => {
 ...
 <Icon d={ICONS.calendar} size={18} className="text-indigo-600 " />
 </div>
 <h2 className="text-lg font-semibold text-gray-900 ">Weekly Schedule</h2>
 </div>
 <div className="text-sm font-medium text-gray-500 ">Current Week</div>
 </div>
 ...
 {DAYS.map(day => (
 <div key={day} className="flex-1 py-2 text-center text-sm font-bold text-gray-700 border-r border-gray-200 last:border-0">
 {day}
 </div>
 ))}
 </div>
 ...
 {/* Time Label */}
 <div className="w-16 shrink-0 py-3 pr-2 text-right text-xs font-medium text-gray-500 border-r border-gray-200 ">
 ...
 className={`flex-1 min-h-[60px] p-1 border-r border-gray-100 last:border-0 cursor-pointer transition-colors relative
 ${isSelected ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-500' : 'hover:bg-gray-50 '}`}
 >
 {slotEvents.map(e => (
 <div key={e._id} className="mb-1 p-1.5 rounded bg-blue-100 border border-blue-200 text-xs text-blue-800 truncate font-semibold shadow-sm">
 {e.title}
 </div>
 ))}
 ...
 <h3 className="text-lg font-semibold text-gray-900 mb-4">Book Timeslot</h3>
 
 {!selectedSlot ? (
 <div className="text-center py-10 bg-gray-50 rounded-lg border border-dashed border-gray-300 ">
 <Icon d={ICONS.clock} size={32} className="mx-auto text-gray-400 mb-2" />
 <p className="text-sm text-gray-500 ">Select a slot on the calendar to check faculty availability.</p>
 </div>
 ) : (
 ...
 <Icon d={ICONS.calendar} className="text-indigo-600 " />
 <div>
 ...
 <p className="text-xs text-gray-500 ">1 hour duration</p>
 </div>
 </div>
 ...
 className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300 mb-4"
 />
 </div>
 ...
 <p className="text-xs text-gray-500 ">{isBusy ? 'Busy in another event' : 'Available'}</p>
 </div>
 </div>
 {isSelected && (
 <Icon d={ICONS.check} className="text-indigo-600 " />
 )}
 </div>
 );
 })}
 ...
 className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white font-medium rounded-lg transition-colors shadow-sm disabled:cursor-not-allowed"
 >
 Schedule Event
 </button>

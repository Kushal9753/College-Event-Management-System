        ...
        <div className="p-6 md:p-8 overflow-y-auto flex-grow">
          {activeTab === 'details' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
              ...
              <div>
                <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-3 border-b pb-2">Assigned Faculty</h4>
                {event.assignedFaculty?.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {event.assignedFaculty.map(f => (
                      <div key={f._id || f.email} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm hover:shadow-md transition-all">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 rounded-full bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-xl shrink-0">
                            {f.name?.charAt(0).toUpperCase()}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h5 className="text-sm font-bold text-gray-900 dark:text-white truncate">{f.name}</h5>
                            <p className="text-xs text-indigo-600 dark:text-indigo-400 font-medium truncate mb-2.5">{f.designation || 'Assistant Professor'}</p>
                  ...
                  <p className="text-sm bg-gray-50 dark:bg-gray-800 p-3 rounded-lg border dark:border-gray-700 italic text-gray-500">

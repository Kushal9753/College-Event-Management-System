...
const Dashboard = () => {
 ...
 setSelectedEvent(prev => {
  if (!prev) return null;
  return [...created, ...assigned].find(e => e._id === prev._id) || prev;
 });
 setError(null);
 } catch (err) {
 ...
 <p className="text-gray-600 mt-1">Refined event management & real-time analytics.</p>
 </div>
 ...
 {error && <div className="mb-6 px-4 py-3 rounded-xl bg-red-50 text-red-700 border border-red-200 text-sm">{error}</div>}
 ...
 <input type="text" name="search" placeholder="Search events..." value={filters.search} onChange={handleFilterChange} className="w-full pl-10 pr-4 py-2 text-sm bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all " />
 </div>
 <select name="category" value={filters.category} onChange={handleFilterChange} className="w-full md:w-48 py-2 text-sm bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all ">
 ...
 <select name="status" value={filters.status} onChange={handleFilterChange} className="w-full md:w-48 py-2 text-sm bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all ">
 <option value="">All Statuses</option>
 {['pending', 'approved', 'completed', 'rejected'].map(st => <option key={st} value={st}>{st.charAt(0).toUpperCase() + st.slice(1)}</option>)}
 </select>
 </div>
 ...
 {/* Main Events Table */}
 <div>
 <h2 className="text-lg font-bold text-gray-800 mb-4">Your Primary Events</h2>
 ...
 <td className="px-6 py-4 text-center">
 <button onClick={() => setSelectedEvent(event)} className="text-xs font-bold text-indigo-600 hover:text-indigo-800 underline uppercase">Manage →</button>
 </td>
 </tr>
 ))}
 ...
 <div key={event._id} className="bg-white p-6 rounded-2xl border shadow-sm hover:shadow-md transition-all active:scale-[0.98] cursor-pointer" onClick={() => setSelectedEvent(event)}>
 ...
 {assignedEvents.length === 0 && <div className="col-span-full py-8 text-center bg-gray-50 rounded-2xl text-gray-400 text-sm italic">Nothing assigned to you currently.</div>}
 </div>
 </div>
 </div>
 )}

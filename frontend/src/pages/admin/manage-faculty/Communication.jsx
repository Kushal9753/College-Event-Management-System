...
const Communication = () => {
 ...
 <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${msg.type === 'announcement' ? 'bg-amber-100 text-amber-800 ' : 'bg-blue-100 text-blue-800 '}`}>
 {msg.type}
 </span>
 </td>
 <td className="px-6 py-4 text-gray-800 truncate max-w-xs">{msg.message}</td>
 ...
 className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-violet-500 outline-none"
 >
 ...
 <label className="block text-sm font-medium text-gray-700 ">Recipients ({formData.receivers.length} selected)</label>
 <button type="button" onClick={selectAllRecipients} className="text-xs text-violet-600 hover:text-violet-800 ">Select All</button>
 </div>
 ...
 <label key={faculty._id} className="flex items-center gap-3 p-2 hover:bg-gray-200 rounded cursor-pointer">
 <input
 type="checkbox"
 ...
 />
 <span className="text-sm text-gray-700 ">{faculty.name} ({faculty.department})</span>
 </label>
 ))
 )}
 </div>
 ...
 rows={4}
 className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-violet-500 outline-none resize-none"
 ...
 <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-gray-600 hover:bg-gray-100 rounded-lg font-medium">Cancel</button>

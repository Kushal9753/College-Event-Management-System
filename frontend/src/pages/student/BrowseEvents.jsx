...
const BrowseEvents = () => {
 ...
 if (['approved', 'ongoing', 'completed'].includes(newEvent.status)) {
 setEvents((prev) => [newEvent, ...prev]);
 }
 });

 socket.on('event_updated', (updatedEvent) => {
 if (['approved', 'ongoing', 'completed'].includes(updatedEvent.status)) {
 setEvents((prev) => {
 ...
 className="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-xl leading-5 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-shadow duration-200"
 />
 </div>
 </div>
 ...
 {[...Array(6)].map((_, i) => (
 <div key={i} className="animate-pulse bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-64">
 ...
 <div
 key={event._id}
 className="bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 overflow-hidden flex flex-col transition-all duration-300 transform hover:-translate-y-1"
 >
 ...
 {/* Fee & Registration Badge */}
 <div className="flex items-center gap-2 pt-2">
 <span className={`text-xs font-bold px-2 py-0.5 rounded ${event.registrationFees > 0 ? 'bg-indigo-100 text-indigo-700' : 'bg-green-100 text-green-700'}`}>
 {event.registrationFees > 0 ? `₹${event.registrationFees}` : 'Free'}
 </span>
 {event.isRegistered && (
 <span className="text-xs font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-700">✓ Registered</span>
 )}
 </div>
 ...
 className="w-full text-center text-sm font-medium text-blue-600 hover:text-blue-700 hover:bg-blue-50 py-2.5 rounded-xl border border-blue-100 transition-colors"
 >
 View Details
 </button>
 </div>
